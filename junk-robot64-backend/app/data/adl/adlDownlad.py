...
from app.data.adl.adlSecrets import *
...
def readArgs():
  ...
  if (nbr >= 2):
    ...
    print(LEV2, "USE: AdlCopy.py <source> <target> [<debug>]")
    sys.exit(1)
...
def initializeAdls():
  ...
  log.debug("%sadlsRoot: %s", LEV2, adlsRoot)
...
def AzCopy( adl, source, target ):
  ...
  multithread.ADLDownloader(adl, rpath=source, lpath=target)
